(function () {
    ...
    async function chatWithOptions(messages, options, fetchOverride) {
        ...
        function finalizeTransport(result, extra) {
            ...
            try {
                if (setup.AIRuntimeSettings && typeof setup.AIRuntimeSettings.noteProviderResult === "function") {
                    setup.AIRuntimeSettings.noteProviderResult(result);
                }
            } catch (error) { /* Provider health UI must never break transport. */ }
            return result;
        }
