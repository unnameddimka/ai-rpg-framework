(function () {
    ...
    function restorePlayerSetup(candidate, source, report) {
        const rawSetup = source && source.playerSetup;
        if (rawSetup && typeof rawSetup === "object" && rawSetup.completed === true && rawSetup.mode === "authored") {
            ...
            if (validCustomTravelerAuthoring(fallbackIdentity)) {
                const applied = applyTravelerIdentity(candidate, fallbackIdentity);
                if (!applied.ok) throw new Error(applied.error.message);
                candidate.playerSetup = {
                    disclaimerAccepted: true,
                    disclosureVersion: 0,
                    aiSetupAcknowledged: true,
                    completed: true,
                    mode: "custom",
                    customAuthoring: clone(fallbackIdentity)
                };
                report.warnings.push("Legacy authored Traveler profile was materialized as self-contained Custom Traveler authoring.");
                return;
            }
            candidate.playerSetup = { disclaimerAccepted: true, disclosureVersion: 0, aiSetupAcknowledged: true, completed: true, mode: "legacy", customAuthoring: null };
            report.warnings.push("Legacy authored Traveler identity was invalid; current generic Traveler identity was used.");
            return;
        }

        const setupState = normalizePlayerSetup(rawSetup, true);
    ...
    function migrateSavedWorld(savedWorld) {
        ...
        try {
            ...
            if (source.calendar && Number.isInteger(source.calendar.dayNumber) && source.calendar.dayNumber >= 0) {
                candidate.calendar.dayNumber = source.calendar.dayNumber;
                Object.values(candidate.entities).filter(function (entity) {
                    return entity && entity.type === "item" && entity.tradeProvenance && entity.tradeProvenance.role === "sale_stock";
                }).forEach(function (item) { removeItemFromCandidate(candidate, item.id); });
            }
            ...
            savedItems.forEach(function (savedItem) {
                ...
                if (definition.writable === true && typeof migratedItem.content !== "string") migratedItem.content = "";
