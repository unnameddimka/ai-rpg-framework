(function () {
    ...
    const ROUND_COUNT = 5;
    const STRUCTURAL_REQUEST_OPTIONS = Object.freeze({ maxTokens: 1200, reasoningMaxTokens: 0, reasoningEffort: "none", temperature: 0.2 });
    const INTENT_REQUEST_OPTIONS = Object.freeze({ maxTokens: 700, reasoningMaxTokens: 0, reasoningEffort: "none", temperature: 0.3 });
    const RESOLVER_REQUEST_OPTIONS = Object.freeze({ maxTokens: 1000, reasoningMaxTokens: 0, reasoningEffort: "none", temperature: 0.3 });
    const REFLECTION_REQUEST_OPTIONS = Object.freeze({ maxTokens: 1800, reasoningMaxTokens: 0, reasoningEffort: "none", temperature: 0.4 });
    let inFlight = false;
    ...
    async function requestStructured(spec) {
        ...
        return execute({
            ...
            requestOptions: clone(spec.requestOptions || STRUCTURAL_REQUEST_OPTIONS),
            client: spec.client || setup.OpenRouterClient,
            run: async function (policyClient) {
                let currentMessages = clone(baseMessages);
                let requestOptions = clone(spec.requestOptions || STRUCTURAL_REQUEST_OPTIONS);
    ...
    function plannerSystem() {
        return [
            "You are planning coarse activity for exactly one RPG character during a timelapse. The current mode is overnight.",
    ...
    function compactPrivateContext(characterId, pendingObservations) {
        const base = setup.ContextBuilder.build(characterId, { pendingObservations: pendingObservations || [] });
        ...
        return {
            ...
            view: {
                self: {
                    ...
                    sublocation_id: self.sublocation_id || null,
                    sleeping: self.sleeping === true,
                    position_text: self.position_text || "",
                    inventory: (self.inventory || []).map(function (item) {
                        return { id: item.id, name: item.name, definition_id: item.definition_id, tags: clone(item.tags || []) };
                    })
                },
                location: { id: location.id || self.location_id || null, name: location.name || locationName(self.location_id) }
            },
            character: clone(base.character || {}),
            mind: clone(base.mind || {}),
            continuation: base.continuation === undefined ? null : clone(base.continuation),
            pendingObservations: clone(base.pendingObservations || [])
        };
    }
    ...
    async function requestPlan(characterId, startRound, remainingRounds, facts, latestEncounter, latestFailure, client, mode) {
        ...
        return requestStructured({
            ...
            requestOptions: STRUCTURAL_REQUEST_OPTIONS,
            client: client,
    ...
    async function requestInteractionIntent(characterId, round, locationId, ownActivity, otherActivities, facts, client, mode) {
        ...
        return requestStructured({
            ...
            messages: messages,
            contract: contract,
            requestOptions: INTENT_REQUEST_OPTIONS,
            client: client,
    ...
    async function requestInteractionResume(round, locationId, participants, activities, intents, publicFacts, client, mode) {
        ...
        return requestStructured({
            ...
            messages: messages,
            contract: contract,
            requestOptions: RESOLVER_REQUEST_OPTIONS,
            client: client,
    ...
    async function requestReflection(characterId, facts, client, mode) {
        ...
        return requestStructured({
            ...
            requestOptions: REFLECTION_REQUEST_OPTIONS,
            client: client,
    ...
    async function runTimelapseCore(client, options) {
        ...
        const roundCount = Number.isInteger(options.roundCount) && options.roundCount > 0 ? options.roundCount : ROUND_COUNT;
    ...
    async function runOvernight(client, options) {
        ...
        inFlight = true;
        try {
            const result = await runTimelapseCore(client, Object.assign({}, options, { mode: MODE, roundCount: ROUND_COUNT }));
