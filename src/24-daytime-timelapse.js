(function () {
    ...
    async function settleActivity(record, committedFacts, client) {
        const world = currentWorld();
        const activity = record.definition;
        const active = record.active;
        ...
        try {
            ...
            if (activity.settlement.type === "random_items") {
                const count = activity.settlement.minTotal + Math.floor(Math.random() * (activity.settlement.maxTotal - activity.settlement.minTotal + 1));
            ...
            return { ok: true, records: [{ text: text, actorIds: actorIds, locationId: activity.workLocationId, kind: "day_activity_settlement", visibleToHuman: true }] };
        } catch (error) {
        ...
        try {
            const coreOptions = Object.assign({}, options, {
                ...
                beforeReflection: async function (details) {
                    return settleActivity(record, details.committedFacts, client);
                }
            });
