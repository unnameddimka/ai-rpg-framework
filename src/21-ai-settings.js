(function () {
    ...
    const models = catalog.models.map(function (model) {
        return { id: String(model.id), name: String(model.name), roles: Array.isArray(model.roles) ? model.roles.map(String) : [] };
    });
    const modelById = {};
    ...
    function eligible(modelId, role) {
        const model = modelById[modelId];
        return Boolean(model && Array.isArray(model.roles) && model.roles.includes(role));
    }
    ...
    function clearKeyRejection() {
        keyRejected = false;
        keyRejectedDetails = null;
    }

    function noteProviderResult(result) {
        if (result && result.ok) {
            clearKeyRejection();
        } else if (result && result.error && result.error.code === "AUTHENTICATION_FAILED") {
            keyRejected = true;
            keyRejectedDetails = { code: result.error.code, status: result.status || 401 };
        }
        return { rejected: keyRejected };
    }
    ...
    function readSaved(storage, now) {
        ...
        try {
            ...
            if (savedModelId && eligible(savedModelId, "character")) {
            ...
            if (savedNarratorModelId && eligible(savedNarratorModelId, "narrator")) {
            ...
            if (savedUtilityModelId && eligible(savedUtilityModelId, "utility")) {
    ...
    function selectModel(modelId, storage) {
        ...
        if (!eligible(normalized, "character")) {
            return { ok: false, error: { code: "UNKNOWN_MODEL", message: "Choose a Character model from the supported model list." } };
        }
    ...
    function selectNarratorModel(modelId, storage) {
        const normalized = typeof modelId === "string" ? modelId.trim() : "";
        if (!eligible(normalized, "narrator")) {
            return { ok: false, error: { code: "UNKNOWN_MODEL", message: "Choose a Narrator model from the supported model list." } };
        }
    ...
    function selectUtilityModel(modelId, storage) {
        const normalized = typeof modelId === "string" ? modelId.trim() : "";
        if (!eligible(normalized, "utility")) {
            return { ok: false, error: { code: "UNKNOWN_MODEL", message: "Choose a Utility model from the supported model list." } };
        }
    ...
    setup.AIRuntimeSettings = {
        ...
        getModelsForRole: modelsForRole,
        ...
        getStatus: function () {
            ...
            return {
                ...
                keyStatus: keyRejected && apiKey ? "rejected" : (apiKey ? "available" : "not_set"),
                keyRejected: keyRejected,
                keyRejectedDetails: clone(keyRejectedDetails),
                warning: warning,
        ...
        noteProviderResult: noteProviderResult,
        clearKeyRejection: clearKeyRejection,
        forget: forget
    };
}());
