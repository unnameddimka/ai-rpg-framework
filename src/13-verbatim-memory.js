(function () {
    ...
    function compactRecord(record) {
        ...
        const authority = A.normalize(record.worldStateAuthority);
        if (authority) output.worldStateAuthority = authority;
        if (Array.isArray(record.epistemicParts) && record.epistemicParts.length) output.epistemicParts = clone(record.epistemicParts);
    ...
    function appendFromObservation(characterId, observation, world) {
        ...
        return append(characterId, {
            ...
            worldStateAuthority: A.forObservation(observation),
            epistemicParts: Array.isArray(observation.epistemicParts) && observation.epistemicParts.length
                ? clone(observation.epistemicParts)
                : (setup.EventPerception && typeof setup.EventPerception.observationEpistemicParts === "function"
                    ? setup.EventPerception.observationEpistemicParts(characterId, observation, world) : undefined),
    ...
    function appendOwnEvent(characterId, event, world) {
        ...
        return append(characterId, {
            ...
            text: event.text,
            worldStateAuthority: A.forEvent(event),
            epistemicParts: setup.EventPerception && typeof setup.EventPerception.eventEpistemicParts === "function"
                ? setup.EventPerception.eventEpistemicParts(event, characterId) : undefined,
    ...
    function appendTimelapseExperience(characterId, sequenceId, text, metadata, world) {
        ...
        return append(characterId, {
            ...
            text: text,
            worldStateAuthority: A.forTimelapse(metadata.kind || "timelapse_experience", metadata),
            epistemicParts: [{
                type: A.forTimelapse(metadata.kind || "timelapse_experience", metadata) === "narrative_only" ? "direct_observation" : "formal_fact",
                actorId: metadata.actorId || characterId,
                text: text
            }],
    ...
    setup.VerbatimMemory = {
        ...
        withWorldStateAuthority: function (record, world) { return A.withRecordAuthority(record, currentOr(world)); }
    };
}());
