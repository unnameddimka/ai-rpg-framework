...
Global environment state now exposes coarse time (`Evening`, `Night`, `Morning`, `Day`) plus a saved narrative weather description to every character and to the main UI. New games begin in Evening. Ordinary ticks do not move the clock. Night timelapse ends in Morning; accepted day work or solo hunting runs daytime timelapse and ends in Evening. Weather is refreshed at new-world/legacy initialization and after successful coarse-time transitions through browser-CORS `ipwho.is` approximate IP geolocation + Open-Meteo + a narrowly scoped Narrator request, with a non-blocking saved/fixed fallback. Weather keeps explicit stage diagnostics and is independent of the optional presentation-Narrator toggle.
