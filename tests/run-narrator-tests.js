...
async function testRuntimeTransportBudgetsHotSwitchingAndLogging() {
    ...
    assert(!switched.ok && settings.getSelectedModelId() === characterModelId && settings.getSelectedNarratorModelId() === "sao10k/l3.3-euryale-70b:nitro",
        "unsupported narrator model switches should be rejected without changing either role");
    ...
    assert(tickResult.ok && bodies[1].model === "sao10k/l3.3-euryale-70b:nitro" && bodies[1].max_tokens === 700 &&
        bodies[1].temperature === 0.7 && !Object.prototype.hasOwnProperty.call(bodies[1], "reasoning"),
        "narrator requests should continue using the supported Euryale role model");
    ...
    assert(narrationEntries.length === 2 && narrationEntries[0].request.stage === "location" && narrationEntries[1].request.stage === "tick" &&
        narrationEntries[1].request.modelId === "sao10k/l3.3-euryale-70b:nitro" &&
