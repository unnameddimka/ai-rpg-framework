...
function sleepingPlanFor(actorId) {
    ...
    if (actorId === "blacksmith") {
        return [{ locationId: "villageSmithy", action: { type: "sleep", bedId: "smithyLivingBed" } }];
    }
    if (actorId === "nell") {
...
async function main() {
    ...
    ["player", "innkeeper", "nell", "hoodedWoman", "blacksmith"].forEach(function (id) {
        assert(known(world.entities[id], "village_name"), `${id} should know that the village is called Mallowstead`);
    });
    ...
    const planCatalog = catalogFor("blacksmith");
    const garrickCatalog = catalogFor("innkeeper");
    const playerCatalog = catalogFor("player");
    assert(room(planCatalog, "villageSmithy") && room(planCatalog, "villageSmithy").beds.some(function (bed) { return bed.id === "smithyLivingBed"; }),
        "Harlan should be able to route to his authored sleeping place at the smithy");
    assert(room(garrickCatalog, "guestRoom1") && room(garrickCatalog, "guestRoom1").beds.some(function (bed) { return bed.id === "guestRoom1Bed"; }),
        "Garrick should route through locked Guest Room 1 because he carries its key in the public world");
    ...
    assert(world.entities.player.aiDescription.includes("Do not assume a particular current location or activity") &&
        ...
        world.entities.blacksmith.aiDescription === setup.GeneratedWorldData.characters.blacksmith.aiDescription &&
    ...
    const validEarlySleep = setup.NightTimelapse.validatePlan({ steps: [
        { locationId: "villageSmithy", action: { type: "sleep", bedId: "smithyLivingBed" } }
    ] }, planCatalog, 5);
    ...
    assert(!setup.NightTimelapse.validatePlan({ steps: [
        { locationId: "villageSmithy", action: { type: "sleep", bed_id: "smithyLivingBed" } }
    ] }, planCatalog, 5).ok, "sleep must use the exact bedId field rather than bed_id");
    ...
    }) }, planCatalog, 5).ok, "a plan without sleep should provide exactly every remaining round");
    assert(!setup.NightTimelapse.validatePlan({ steps: [
        { locationId: "commonRoom", action: { type: "narrate", text: "Wait quietly." } }
    ] }, planCatalog, 5).ok, "a non-sleeping short plan should be rejected");
    assert(setup.NightTimelapse.validatePlan({ steps: [
        { locationId: "commonRoom", action: { type: "timelapse_action", actionId: "clean_common_room" } },
        { locationId: "villageSmithy", action: { type: "sleep", bedId: "smithyLivingBed" } }
    ] }, planCatalog, 5).ok, "authored macros should use the exact timelapse_action/actionId branch");
    assert(!setup.NightTimelapse.validatePlan({ steps: [
        { locationId: "commonRoom", action: { type: "clean_common_room" } },
        { locationId: "villageSmithy", action: { type: "sleep", bedId: "smithyLivingBed" } }
    ] }, planCatalog, 5).ok, "authored action IDs must not be used as action types");
    assert(!setup.NightTimelapse.validatePlan({ steps: [
        { locationId: "villageSmithy", action: { type: "sleep", bedId: "smithyLivingBed" } },
        { locationId: "commonRoom", action: { type: "narrate", text: "This must not follow sleep." } }
    ] }, planCatalog, 5).ok, "no plan step may follow sleep");

    // The reusable core must not own overnight-only HumanController postconditions.
    world = fresh();
    ["innkeeper", "blacksmith", "nell", "hoodedWoman"].forEach(function (id) {
    ...
    world.entities.heldAleFixture = { id: "heldAleFixture", type: "item", definitionId: "mugOfAle", name: "Mug of ale", containerId: "inventory_blacksmith" };
    world.inventories.inventory_blacksmith.itemIds.push("heldAleFixture");
    ...
    assert(cleanup.affectedItemIds.length === 3 && ["tableTwoAle_1", "tableTwoAle_2", "tableTwoAle_3"].every(function (id) {
    ...
    assert(world.entities.heldAleFixture.definitionId === "mugOfAle" && world.entities.heldAleFixture.containerId === "inventory_blacksmith",
    ...
    world = fresh();
    ["innkeeper", "blacksmith", "nell"].forEach(function (id) {
    ...
    world.entities.player.sleeping = true;
    place("innkeeper", "commonRoom", "commonRoomFloor");
    place("blacksmith", "commonRoom", "commonRoomTableTwo");
    place("nell", "commonRoom", "underStairsNook");
    place("hoodedWoman", "secludedCottage", "maraCottageBed");
    world.entities.hoodedWoman.sleeping = true;
    ["innkeeper", "blacksmith", "nell", "hoodedWoman"].forEach(function (id) { world.entities[id].mind.pendingObservations = []; });
    world.ai.turnQueue = [];
    const noOpStages = [];
    const noOpClient = {
        enforceRequestTiming: false,
    ...
    const fakeClient = {
        ...
        chat: async function (messages, requestOptions) {
            ...
            if (payload.stage === "timelapse-interaction-resolver") {
                ...
                return response({ interactionOccurred: true, interactionResume: "Garrick, Harlan, and Nell exchanged a brief greeting in the common room and verbally ended the conversation." });
            }
            if (payload.stage === "timelapse-replan") {
                const actorId = payload.context.view.self.id;
                assert(payload.context.timelapse.startRound === 2 && payload.context.timelapse.remainingRounds === 4 &&
    ...
    assert(world.entities.innkeeper.sleeping === true && world.entities.innkeeper.sublocationId === "innkeeperRoomBed" &&
        world.entities.blacksmith.sleeping === true && world.entities.blacksmith.sublocationId === "smithyLivingBed" &&
    ...
    world.entities.player.sleeping=true;
    const maintenanceIds=["innkeeper","nell","hoodedWoman","blacksmith","roadMerchant"];
