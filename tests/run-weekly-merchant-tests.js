...
assert(merchant.awayState && merchant.awayState.present === true && merchant.awayState.plannedDeparture.dayNumber === 1 && merchant.awayState.plannedDeparture.phase === "Morning", "fresh Monday Evening should initialize a Flamesday Morning planned departure");
...
assert(setup.WeeklyRhythm.currentWeekdayName(world) === "Flowday" && !merchant.awayState.present && merchant.awayState.travelPeriodsRemaining === 2, "Flowday Morning should reflect exactly one completed road period");
const roadTwo = ok(setup.WeeklyRhythm.advanceEveningBoundary(world), "Flowday Day completes road period 2");
assert(!merchant.awayState.present && merchant.awayState.travelPeriodsRemaining === 1, "Flowday Evening should reflect exactly two completed road periods");
...
assert(staying.awayState.present && staying.awayState.plannedDeparture.dayNumber === 3 && staying.awayState.plannedDeparture.phase === "Evening", "continuous stay should preserve the actively deferred departure rather than reset it on Woodsday");
