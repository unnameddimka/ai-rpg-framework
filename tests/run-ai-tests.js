...
function queueHooded() {
    const world = fresh();
    ok(setup.Game.assignNonHumanController("blacksmith", "dummy"), "isolated queue fixture disables Harlan AI");
    ok(setup.Game.assignNonHumanController("nell", "dummy"), "isolated queue fixture disables Nell AI");
    ok(setup.Game.assignNonHumanController("roadMerchant", "dummy"), "isolated queue fixture disables Maksym AI");
...
load("src/22-openrouter-client.js"); load("src/23-ai-protocol.js"); load("src/23-structured-ai-request.js"); load("src/23-z-action-contract-repair.js"); load("src/24-ai-request-executor.js"); load("src/24-item-model-effects.js"); load("src/24-ai-turn-scheduler.js"); load("src/20-controllers.js"); load("src/24-prompt-lab.js"); load("src/25-turn-flow.js");
...
async function main() {
    ...
    assert(failedHumanAttempt.ok && failedHumanAttempt.turnConsumed === true &&
        ...
        failedHumanAttempt.intentResult.actionResult.error.code === "PASSAGE_LOCKED" &&
    ...
    const maraProgressResult = { ok: true, actorId: "hoodedWoman", narrativeText: "Mara gives a brief nod.", intentResult: { narrativeResult: null }, actionResult: null };
    ...
    setup.AITurnScheduler.processAfterSubmit = async function (client, options) {
        options.onCommittedResult(maraProgressResult);
        await new Promise(function (resolve) { setTimeout(resolve, 5); });
        options.onCommittedResult(nellProgressResult);
        return { ok: true, processedCount: 2, reactedCharacterIds: ["hoodedWoman", "nell"], results: [maraProgressResult, nellProgressResult], remainingQueue: setup.AITurnScheduler.getQueueView() };
    };
    ...
    assert(progressiveTickBatches.length === 3 && progressiveTickBatches[0].meta.phase === "human" &&
        progressiveTickBatches[1].meta.phase === "ai-reaction" && progressiveTickBatches[1].meta.actorId === "hoodedWoman" &&
    ...
    assert(progressiveTickResult.narrativeFragments.length === 3 &&
        progressiveTickResult.narrativeFragments.filter(function (text) { return text.includes("Mara gives a brief nod"); }).length === 1,
    ...
    const attributedNarrative = setup.TurnFlow.describeAIResult({
        ok: true,
        actorId: "hoodedWoman",
        narrativeText: "*Studies the newcomer from beneath the hood.*",
        intentResult: { narrativeResult: null },
        actionResult: null
    }, "player");
    assert(attributedNarrative.visible.length === 1 &&
        attributedNarrative.visible[0] === "Mara the Hedge Witch: *Studies the newcomer from beneath the hood.*" &&
        attributedNarrative.entries[0].actorName === "Mara the Hedge Witch",
    ...
    const rejectedUtilitySelection = setup.AIRuntimeSettings.selectUtilityModel("deepseek/deepseek-v4-pro", storage);
    assert(!rejectedUtilitySelection.ok && setup.AIRuntimeSettings.getSelectedUtilityModelId() === "deepseek/deepseek-v4-flash" &&
        setup.AIRuntimeSettings.getSelectedModelId() === characterBeforeUtilitySelection,
        "Utility should currently be restricted to DeepSeek V4 Flash and remain independent from the character selector");
    const characterModelBeforeNarratorSelection = setup.AIRuntimeSettings.getSelectedModelId();
    const rejectedNarratorSelection = setup.AIRuntimeSettings.selectNarratorModel("deepseek/deepseek-v4-pro", storage);
    assert(!rejectedNarratorSelection.ok && setup.AIRuntimeSettings.getSelectedNarratorModelId() === "sao10k/l3.3-euryale-70b:nitro" &&
        setup.AIRuntimeSettings.getSelectedModelId() === characterModelBeforeNarratorSelection,
        "Narrator should currently be restricted to Euryale and remain independent from the selected character model");
    const flashId = "deepseek/deepseek-v4-flash";
    const selectedFlash = setup.AIRuntimeSettings.selectModel(flashId, storage);
    assert(selectedFlash.ok && selectedFlash.persisted && storageData[setup.AIRuntimeSettings.MODEL_STORAGE_KEY] === flashId &&
        setup.AIRuntimeSettings.getSelectedModel().name === "DeepSeek V4 Flash",
        "Character selection should allow explicitly selecting the default Flash model");
    const rejectedModel = setup.AIRuntimeSettings.selectModel("unknown/model", storage);
    assert(!rejectedModel.ok && rejectedModel.error.code === "UNKNOWN_MODEL" && setup.AIRuntimeSettings.getSelectedModelId() === flashId,
        "unknown models must be rejected without changing the active Character selection");
    storageData[setup.AIRuntimeSettings.MODEL_STORAGE_KEY] = "sao10k/l3.3-euryale-70b:nitro";
    setup.AIRuntimeSettings.readSaved(storage, 1001);
    assert(setup.AIRuntimeSettings.getSelectedModelId() === "deepseek/deepseek-v4-flash" && !storageData[setup.AIRuntimeSettings.MODEL_STORAGE_KEY],
        "a saved model that is not eligible for the Character role should fall back to the current Character default");
    setup.AIRuntimeSettings.selectModel(flashId, storage);
    ...
    for (const pair of [[401,"AUTHENTICATION_FAILED"],[402,"INSUFFICIENT_CREDITS"],[429,"RATE_LIMITED"],[503,"PROVIDER_UNAVAILABLE"]]) {
        if (pair[0] === 402) ok(setup.AIRuntimeSettings.save(sentinel, false, storage, 1000), "new key entry should clear prior rejection before credit failure test");
        ...
        if (pair[0] === 401) assert(setup.AIRuntimeSettings.getStatus().keyStatus === "rejected", "confirmed OpenRouter 401 should mark the current key Rejected");
        if (pair[0] === 402) assert(setup.AIRuntimeSettings.getStatus().keyStatus === "available", "OpenRouter 402 insufficient credits should keep a configured key Available rather than marking it Rejected");
    }
    ...
    assert(decisionPrompt.includes("spokenLoudness is per utterance, not persistent state") &&
        decisionPrompt.includes('must be "noticeable" or "hidden" or "shout"') &&
        decisionPrompt.includes("Choose spokenLoudness structurally for this utterance") &&
        decisionPrompt.includes("writing *whispers*, *in a low voice*, *shouts*, or similar prose does not change mechanical loudness") &&
        decisionPrompt.includes('"shout" has no addressee') &&
        decisionPrompt.includes("current location plus directly adjacent authored locations") &&
        decisionPrompt.includes("destination addressee is allowed only when") &&
    ...
    assert(decisionPrompt.includes("ongoing role-playing scene") &&
        ...
        decisionPrompt.includes("Narrative is never an alternate execution channel") &&
        ...
        decisionPrompt.includes("Never narrate a completed tracked state change merely because its formal action is currently unavailable") &&
        decisionPrompt.includes("EXTERNAL PREREQUISITE WAIT RULE") &&
        decisionPrompt.includes("Waiting for another character to satisfy a tracked prerequisite is a valid deliberate no-op") &&
        decisionPrompt.includes("the mug is still in that character's possession") &&
        decisionPrompt.includes("A relevant mechanic may require a prerequisite formal action first") &&
        decisionPrompt.includes("Cosmetic or untracked behavior may still be described narratively") &&
        decisionPrompt.includes("must not narratively claim that a second grounded action also completed") &&
    ...
    const moveSpeechCatalog = clone(available);
    moveSpeechCatalog.move.options.speech_targets_by_destination = { bar: [{ id: "hoodedWoman", name: "Mara" }] };
    assert(setup.AIProtocol.validateDecision({ action: { type: "move", destination_id: "bar" }, publicNarrative: null, spokenText: "Mara, wait.", spokenTargetId: "hoodedWoman", spokenLoudness: "noticeable", continuation: null, memoryUpdates: emptyUpdates() }, moveSpeechCatalog, []).ok &&
        !setup.AIProtocol.validateDecision({ action: { type: "move", destination_id: "bar" }, publicNarrative: null, spokenText: "Hey!", spokenTargetId: null, spokenLoudness: "shout", continuation: null, memoryUpdates: emptyUpdates() }, moveSpeechCatalog, []).ok &&
        !setup.AIProtocol.validateDecision({ action: null, publicNarrative: null, spokenText: "Hey!", spokenTargetId: "hoodedWoman", spokenLoudness: "shout", continuation: null, memoryUpdates: emptyUpdates() }, moveSpeechCatalog, ["hoodedWoman"]).ok &&
        setup.AIProtocol.validateDecision({ action: null, publicNarrative: null, spokenText: "Hey!", spokenTargetId: null, spokenLoudness: "shout", continuation: null, memoryUpdates: emptyUpdates() }, moveSpeechCatalog, []).ok,
        "AI protocol should allow only grounded move-destination addressees and enforce stationary targetless shout semantics");
    const canonicalLoudness = setup.CharacterAPI.getSpeechLoudnessValues();
    assert(JSON.stringify(canonicalLoudness) === JSON.stringify(["noticeable", "hidden", "shout"]) &&
    ...
    assert(exportedExchange.ok && exportedExchange.filename === "ai-rpg-ai-exchange-20260804-190000Z.json" &&
        ...
        exportedExchange.data.runtime.model === flashId &&
    ...
    // 0.1.2b keeps action-contract handling single-pass: no detector/Utility repair call is inserted after the Character decision.
    world = fresh();
    ok(setup.Game.assignNonHumanController("innkeeper", "ai"), "single-pass contract fixture enables Garrick AI");
    ok(setup.CharacterAPI.perform("player", { type: "move", destination_id: "commonRoom" }), "single-pass contract fixture enters common room");
    ok(setup.CharacterAPI.perform("player", { type: "move", destination_id: "bar" }), "single-pass contract fixture approaches Garrick at the bar");
    ok(setup.CharacterAPI.narrate("player", { text: "Two ales, please.", target_id: "innkeeper" }), "single-pass contract fixture queues Garrick");
    let contractPassCalls = 0;
    const singlePassTurn = await setup.AIController.takeQueuedTurn("innkeeper", {
        chat: async function () {
            contractPassCalls += 1;
            return response({
                action: null,
                publicNarrative: "Garrick considers the request and reaches toward the mug cabinet.",
                spokenText: "One moment.",
                memoryUpdates: emptyUpdates()
            });
        }
    });
    assert(singlePassTurn.ok && contractPassCalls === 1,
        "ordinary Character reaction must complete with exactly one Character model call and no action-contract repair pass");
